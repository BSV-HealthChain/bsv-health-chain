...
const ProviderDashboard: React.FC = () => {
  ...
  const handleProviderSubmit = async (data: ProviderFormData) => {
    ...
    try {
      ...
      if (result.error) setLastMessage("Failed to submit: " + result.error);
      else setLastMessage("Provider information submitted successfully!");
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 py-8">
      ...
      <div className="max-w-[1200px] mx-auto px-4 flex flex-col gap-6">
        ...
        <div className="flex gap-3 mb-6">
          {["form", "lookup", "consultations", "records"].map((tab) => (
            <button
              key={tab}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xl">
          {activeTab === "form" && <ProviderForm onSubmit={handleProviderSubmit} />}
          {activeTab === "lookup" && <LookupPanel />}
          {activeTab === "consultations" && pubKey && <ConsultationsTab pubKey={pubKey} />}
          {activeTab === "records" && pubKey && <RecordsTab pubKey={pubKey} />}
        </div>
